...
class CreateTolokaProjectTranslate(BaseProcessor):
    def __init__(
        self,
        API_KEY: str,
        platform: str,
    ...
    def process(self):
        ...
        # Input text view
        input_text_view = toloka.client.project.template_builder.TextViewV1(
            toloka.client.project.template_builder.InputData('input_text'), label='Текст'
        )
        # Output text field for translation or any text output
        output_text_field = toloka.client.project.template_builder.TextFieldV1(
            toloka.client.project.template_builder.OutputData('output_text'),
            label='Перевод на армянский.',
        ...
        project_interface = toloka.client.project.TemplateBuilderViewSpec(
            view=toloka.client.project.template_builder.ListViewV1(items=[input_text_view, output_text_field]),
            plugins=[width_plugin],
        )

        input_specification = {'input_text': toloka.client.project.StringSpec()}
        output_specification = {'output_text': toloka.client.project.StringSpec()}
