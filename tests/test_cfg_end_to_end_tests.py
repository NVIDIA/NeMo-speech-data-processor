...
data_check_fn_mcv = partial(
    data_check_fn_generic, file_name=lambda archive_file_stem, **kwargs: f"{archive_file_stem}.tar.gz"
)
data_check_fn_mtedx = partial(
    data_check_fn_generic, file_name=lambda language_id, **kwargs: f"mtedx_{language_id}.tgz"
)
...
def get_test_cases() -> List[Tuple[str, Callable]]:
    return [
        ...
        TestCase(
            config_path=f"{DATASET_CONFIGS_ROOT}/portuguese/coraa/config.yaml", data_check_fn=data_check_fn_coraa
        ),
        TestCase(
            config_path=f"{DATASET_CONFIGS_ROOT}/english/slr83/config.yaml", data_check_fn=lambda raw_data_dir: True
        ),
        TestCase(
            config_path=f"{DATASET_CONFIGS_ROOT}/english/coraal/config.yaml", data_check_fn=lambda raw_data_dir: True
        ),
        TestCase(
        ...
        TestCase(
            config_path=f"{DATASET_CONFIGS_ROOT}/armenian/fleurs/config.yaml", data_check_fn=data_check_fn_fleurs
        ),
        TestCase(
        ...
        TestCase(config_path=f"{DATASET_CONFIGS_ROOT}/kazakh/slr140/config.yaml", data_check_fn=data_check_fn_slr140),
        TestCase(
        ...
        TestCase(
            config_path=f"{DATASET_CONFIGS_ROOT}/uzbek/uzbekvoice/config.yaml", data_check_fn=data_check_fn_uzbekvoice
        ),
        TestCase(config_path=f"{DATASET_CONFIGS_ROOT}/uzbek/fleurs/config.yaml", data_check_fn=data_check_fn_fleurs),
        TestCase(
        ...
        TestCase(config_path=f"{DATASET_CONFIGS_ROOT}/arabic/fleurs/config.yaml", data_check_fn=data_check_fn_fleurs),
        TestCase(
...
def get_test_names():
    config_names = [Path(t.config_path).parent.relative_to(DATASET_CONFIGS_ROOT).as_posix() for t in get_test_cases()]
...
def setup_data(request):
    ...
    config_path, data_check_fn, reference_manifest_filename, fields_to_ignore, processors_to_run = (
        request.param.config_path,
